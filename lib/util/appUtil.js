...
var processArgs = function(app, args){
	if(!args.id) throw new Error('the server id cannot be empty');
	if(!args.serverType) throw new Error('the server type cannot be empty');
	if(!args.host) throw new Error('the server host cannot be empty');
	if(!args.port) throw new Error('the server port cannot be empty');
...
var parseArgs = function(args){
	...
	console.log('[INFO ] [%s] app args: %j.', utils.format(), argsMap);
	return argsMap;
};